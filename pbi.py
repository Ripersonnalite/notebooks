...
# Get the directory one level up from the current script's location
parent_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
# Construct the full path to the file
file_path = os.path.join(parent_dir, 'google_an.ini')
# Create a ConfigParser object
config = configparser.ConfigParser()
# Read the INI file
config.read(file_path)
# Get the tracking ID
ga_tracking_id = config.get('settings', 'ga')
...
for line in button_lines:
    ...
    button_text = name.replace("btn_", "").title()
...
card_body = dbc.CardBody(buttons)
...
app.layout = html.Div(
    ...
    [
        dbc.CardHeader(
            dbc.Row([
                ...
                dbc.Col(html.Div("Dashboard of Dashboards", style={'textAlign': 'center', 'fontSize': '24px'}), width=8),
        ...
        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col(
                    dbc.Card(
                        dbc.CardBody([card_body]), style=var_cardstyle
    ...
    style={
        ...
        'margin': '20px',
        'background-image': 'url("./assets/suit.jpeg")',  # Replace with your actual image file name
